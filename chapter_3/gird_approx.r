...
# assume that all probabilities are equaly likely
prob_p <- rep(1, 1000)
# p(x) = choose(9, 6) p^x (1-p)^(9-x)
# we assume that after tossing the globe 9 times we get 6 W
# so this is the probability we get this result given the proportion of W is p
prob_data <- dbinom(6, size = 9, prob = p_grid)
dens(prob_data)

# P(A|B) =  P(B|A) * P(A) / normalizeing constant
posterior <- prob_data * prob_p
# the sum of all possible probabilities is 1
posterior <- posterior / sum(posterior)
dens(posterior)

# taking a sample of all possible probabilities with the probabilitiy of each
